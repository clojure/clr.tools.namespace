(ns clojure.tools.namespace.test-helpers
  ...
  (:import (System.IO Path File FileInfo TextWriter Directory)))               ;;; (java.io Closeable File Writer PrintWriter)
...
(defn create-temp-dir
  ...
  (let [temp-filename (Path/Combine (Path/GetTempPath) name)]             ;;;  temp-file (File/createTempFile name nil)
    (File/Delete temp-filename)                                                               ;;; (.delete temp-file)
    (Directory/Create temp-filename)                                                          ;;; (.mkdirs temp-file)
...
(defn create-file
  ...
  [path contents]
  {:pre [(vector? path)]}
  (let [name (Path/Combine (into-array String path))]                   ;;; ^File file (apply io/file path)
                                                                        ;;; (when-let [parent (.getParentFile file)]
                                                                        ;;; (.mkdirs parent))
    (with-open [wtr (io/text-writer name)]                              ;;;  io/writer file
